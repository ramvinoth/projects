...
<!--		<div class="fl" style="position: relative;">&nbsp;&nbsp;
	...
	<?php if((SES_TYPE == 1 || SES_TYPE == 2) && defined('TWITTED') && TWITTED == 0 && !in_array(PAGE_NAME,array('onbording','profile','changepassword','email_notifications','email_reports'))){
		$osTwAccount = 'VIYA';
		$tweetTexts = array(
			'#VIYA is an Awesome Project Collaboration Tool that gives full visibility and control over your projects',
			'Organize Projects, Tasks, Documents & Meeting Minutes in one place #VIYA'.' @'.$osTwAccount,
			'Share of your ideas, feedbacks, questions and discussions across the team #VIYA'.' @'.$osTwAccount,
			'See what\'s in progress, what needs to be done and what\'s been accomplished #VIYA'.' @'.$osTwAccount,
			'Receive reminders, alert for close deadlines, manage tightly not to exceed budget #VIYA'.' @'.$osTwAccount,
			'Break-down tasks into smaller ones, share documents using google Drive & Dropbox #VIYA'.' @'.$osTwAccount,
			'Just sit back and keep on watching the Activity even while relaxing #VIYA'.' @'.$osTwAccount,
			'Win your customers\'confidence by keeping them informed with daily scrum #VIYA'.' @'.$osTwAccount,
			'Keep your team on their toes by reminding them by automatic emails #VIYA'.' @'.$osTwAccount,
			'Get instant notification on your cell and respond with your inputs in no time #VIYA'.' @'.$osTwAccount,
			'Stay on top and get weekly usage report #VIYA'.' @'.$osTwAccount,
			'Managing Project Effectively with project collaboration tool taming inbox #VIYA'.' @'.$osTwAccount,
			'Get Daily Progress email from team without fail #VIYA'.' @'.$osTwAccount,
			'#VIYA is a Awesome project management tool for You & Your Team'.' @'.$osTwAccount
		);
